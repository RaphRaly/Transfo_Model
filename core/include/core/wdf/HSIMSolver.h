...
public:
    ...
    const NonlinearLeaf& getNonlinearLeaf(int index) const { return nonlinearLeaves_[index]; }

    // ─── Access ME junctions ────────────────────────────────────────────────
    MEJunction& getMEJunction(int index) { return meJunctions_[index]; }

    // ─── Access magnetic junction ───────────────────────────────────────────
    TopologicalJunction<NumMagPorts>& getMagneticJunction() { return magJunction_; }
    ...
    float processSample(float inputVoltage)
    {
        ...
        if (iterCount > 1)
        {
            diagnostics_.computeSpectralRadius(
                b_nl_.data(), v_prev.data(), v_prev_prev.data());
        }
#endif
    ...
    // ─── Configuration ──────────────────────────────────────────────────────
    void setEpsilon(float eps)           { epsilon_ = eps; convergenceGuard_.configure(eps); }
    ...
    const ConvergenceGuard& getConvergenceGuard() const { return convergenceGuard_; }

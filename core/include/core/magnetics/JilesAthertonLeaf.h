...
public:
    ...
    // ─── Configuration ──────────────────────────────────────────────────────
    void configure(float Gamma, float Lambda, const JAParameterSet& params, double sampleRate)
    {
        Gamma_  = Gamma;
        Lambda_ = Lambda;
    ...
    float scatterImpl(float a_m)
    {
        ...
        const double b_m = static_cast<double>(a_m) - 2.0 * static_cast<double>(this->Z_port_)
                          * Lambda_ * kMu0 * dM / (Gamma_ + kEpsilonD);
    ...
    const HysteresisModel<AnhystType>& getHysteresisModel() const { return hystModel_; }

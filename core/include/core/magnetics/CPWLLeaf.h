...
public:
    ...
    float scatterImpl(float a_m)
    {
        ...
        const auto& segments = (direction_ == Direction::ASCENDING) ? segs_asc_ : segs_desc_;
        const int numSegs = (direction_ == Direction::ASCENDING) ? numSegs_asc_ : numSegs_desc_;
    ...
    bool assertWDFeasibility(float Z_min, float Z_max) const
    {
        ...
        auto checkBranch = [&](const auto& segs, int n) -> bool {
            for (int i = 0; i < n; ++i)
            {
                if (std::abs(segs[i].slope) < kEpsilonF) continue;
...
private:
    ...
    static int findSegment(float x, const std::array<CPWLSegmentCoeffs, kMaxSegments>& segs, int numSegs)
    {
    ...
    static void computeAntiderivativeConstants(CPWLSegmentCoeffs* segs, int numSegs)
    {
        if (numSegs < 1) return;
        ...
        for (int j = 1; j < numSegs; ++j)
        {
            ...
            float F_j_at_bp_no_const = 0.5f * segs[j].slope * bp * bp + segs[j].intercept * bp;
